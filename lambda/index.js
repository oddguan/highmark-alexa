...
const DigitalPolicyHandler = {
  canHandle(handlerInput) {
    const { request } = handlerInput.requestEnvelope;
    return request.intent.name === 'DigitalPolicyIntent';
  },
  handle(handlerInput) {
    let repromptOutput = '';
    const ASK_WRITTEN_OR_LISTEN = `To agree to our digital privacy policy, you can either say, written, and I will send the written version of the digital privacy policy to your mobile app. Or you can say, listen, and agreeing to the digital privacy policy through voice. Which one do you prefer?`;
    return handlerInput.responseBuilder
      .speak(ASK_WRITTEN_OR_LISTEN)
      .reprompt(repromptOutput)
      .getResponse();
  ...
  handle(handlerInput) {
    ...
    if (name === 'WrittenPolicyIntent') {
      ...
      if (isLoggedIn) {
        ...
        say =
          'We will ask you several questions about our digital privacy policy. So first, can Highmark Health collect your basic information via online forms? ';
